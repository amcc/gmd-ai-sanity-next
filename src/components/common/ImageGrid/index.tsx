...
type GridItem =
  | ARTWORKS_QUERYResult[number]
  | BIOGRAPHIES_QUERYResult[number]
  | STUDIOS_QUERYResult[number]
  | RelatedArtwork
  | RelatedBiography;
...
const ImageGrid = ({
  ...
  const sortedItems = useMemo(() => {
    ...
    if (selectedOrderOption?.sortFn) {
      return selectedOrderOption.sortFn(filteredItems as ARTWORKS_QUERYResult);
    }
  ...
  const priorityItems = useMemo(() => {
    return items.filter(
      (item): item is ARTWORKS_QUERYResult[number] =>
        ...
        priorityItems.length > 0 && (
          <PriorityArtworkCarousel
            artworks={priorityItems.filter(
              (item: ARTWORKS_QUERYResult[number]) =>
                item && item.slug && item.mainImage?.asset
            )}
            sizes={carouselSizes}
            recent={recent}
          ...
          {displayedItems.map((item: GridItem) => {
            ...
            return (
              <div key={item._id} className={styles.gridItem}>
                ...
                >
                  <ArtworkFigure
                    artwork={item as ARTWORKS_QUERYResult[number]}
                    sizes={gridSizes}
                    recent={recent}
                    showFilters={showFilters}
                    showDate={true}
